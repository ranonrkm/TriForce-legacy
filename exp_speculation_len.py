...
if args.cache == 'streamllm':
    past_key_values = FlashStreamLLMCache(model=model, max_budget=datalen+gamma+250, skip_start_layers=1, start_size=start_size, recent_size=kv_cache_budget - start_size, gamma=gamma)
else:
...
for input_ids in tokenized_prompts:
    ...
    with torch.no_grad():
        ...
        for i in range(1):
            ...
            # write to file
            with open(f"report/flash_spec_len.csv", 'a') as f:
                f.write(f"{datalen},{gamma},{n / (time2 - time1)},{accepted_rate},{(accepted_count)/ draft_count * gamma}\n")