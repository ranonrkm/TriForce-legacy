...
from models.cache_utils import StreamLLMCache, EfficientH2OCache, DejaVuCache, SimpleCache, Cache
...
class GPTNeoXAttention(nn.Module):
    ...
    def forward(
        ...
        seq_len += past_key_values.seq_len
        ...
        # print(query.shape, key.shape, value.shape, position_ids)
        # Compute attention
...
class GPTNeoXModel(GPTNeoXPreTrainedModel):
    def __init__(self, config):
        ...
        self.layers = nn.ModuleList([GPTNeoXLayer(config, layer_idx) for layer_idx in range(config.num_hidden_layers)])
    ...
    def forward(
        ...
        if past_key_values is None:
            raise ValueError("past_key_values is required for decoding")
        else:
            past_length = past_key_values.seq_len if past_key_values is not None else 0
