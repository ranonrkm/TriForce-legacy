...
from .modeling_llama_cache import LlamaYaRNRotaryEmbedding
...
class LlamaAttention(nn.Module):
    ...
    def _init_rope(self):
        ...
        # if hasattr(self.config, "flash") and self.config.flash:
        if False:
            if self.config.rope_scaling is None:
                scaling_type = "linear"
                scaling_factor = 1.0
            if self.config.rope_scaling is None:
                print("Using FlashRotaryEmbedding")
                ...
                if scaling_type == "yarn":
                    # print("Using FlashYaRNRotaryEmbedding")
