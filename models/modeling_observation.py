...
from models.cache_utils import Cache, FlashSimpleCache, GraphFlashSimpleCache, GraphFlashChunkCache, GraphFlashTopKCache, GraphFlashChunkTopKCache, GraphFlashChunkTopKVerificationCache
...
class LlamaAttention(nn.Module):
    ...
    def forward(
        ...
        if query_states.shape[1] == 1 and self.layer_idx == 10:
            ...
            #     chunk_size = 8
            #     chunks = key_states.shape[1] // chunk_size
            #     assert chunks * chunk_size == key_states.shape[1] - 1
            #     chunk_k = key_states[:,:-1].view(1, chunks, chunk_size, self.num_heads, self.head_dim).mean(dim=-3)
            #     chunk_attn = torch.matmul(query_states.permute(0, 2, 1, 3), chunk_k.permute(0, 2, 3, 1)).squeeze(2) / math.sqrt(self.head_dim)
            #     chunk_attn = nn.functional.softmax(chunk_attn, dim=-1, dtype=torch.float32).to(query_states.dtype)
            #     kv_cache.chunks[self.layer_idx] = chunk_attn.squeeze()

            kv_cache.scores[self.layer_idx].append(attn_weights.squeeze())
