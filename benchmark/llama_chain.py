...
from models.cache_utils import SimpleCache, FlashSimpleCache, GraphFlashSimpleCache, GraphSimpleCache, GraphFlashStreamLLMVerificationCache, GraphFlashStreamEvictionCache_V2
...
draft_cache = GraphFlashStreamEvictionCache_V2(draft, start_size=16, recent_size=250-16, gamma=5)
...
for DEC_LEN in DEC_LEN_LIST:
    ...
    for _ in range(WARM_UP):
        graph_engine.graph_draft_inference(input_ids=input_ids, gamma_offset=gamma)
    ...
    pred_token_idx = torch.randint(low=3, high=30000, size=(1, gamma + 1), device=model.device)
    draft_count = 0
    speculation_probs = []
    generated_ids = []
    ...
    for _ in range(WARM_UP):
        test_real_draft(gamma, pred_token_idx)
