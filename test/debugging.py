...
with torch.no_grad():
    ...
    for input_ids in tqdm(tokenized_prompts, desc="Baseline Test"):
        input_ids = input_ids.to(target.device)[:,:prefill - args.gamma]
...
with torch.no_grad():
    ...
    for input_ids in tqdm(tokenized_prompts, desc="Baseline Test"):
        ...
        # prefill
        graph_engine.inference(input_ids=input_ids[:, :prefill - args.gamma])   
        verify_tokens = input_ids[:, prefill - args.gamma: prefill]
        # verify
        tstart = time.time()
        graph_engine.inference(input_ids=verify_tokens)
        latency = time.time() - tstart
        all_latency.append(latency)

    baseline_latency = (sum(all_latency) / len(all_latency)) * 1000
    print(colored(f"[Baseline-Non-Autoregressive] average latency: {baseline_latency} ms", "red"))