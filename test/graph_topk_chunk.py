...
from models.cache_utils import FlashSimpleCache, GraphFlashChunkTopKCache
...
print_config(target, target, prefill, gen_len, gamma, top_k, top_p, temperature, file_path=file_path, method="TopK Chunk", spec_args={'budget': args.budget}, dataset=args.dataset)

budget = int(args.budget * prefill) // 4 * 4
cache = FlashSimpleCache(target, prefill+gen_len+16)
graph_cache = GraphFlashChunkTopKCache(target, max_budget=budget, prefill=prefill, gamma=gamma, chunk_size=4)
