...
def init_process_group(world_size: int, rank: int):
    ...
    _NCCL_BACKEND = NCCLBackend(world_size, rank)
    _WORLD_SIZE = world_size
    print(f"Initialized NCCL backend with world size {_WORLD_SIZE} and rank {rank} {_NCCL_BACKEND}")
...
def all_reduce(input_: torch.Tensor, op=ReduceOp.SUM) -> None:
    ...
    _NCCL_BACKEND.all_reduce(in_array=cupy_input, out_array=cupy_input, op=_OP_MAPPING[op])
    
def barrier() -> None:
    """Barrier for the NCCL backend."""
    _NCCL_BACKEND.barrier()
