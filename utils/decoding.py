...
def shift_tensor(tensor, incoming_tensor):
    ...
    # print(tensor.shape, incoming_tensor.shape, incoming_tokens, tensor[:, incoming_tokens:].shape, incoming_tensor.shape)
    tensor[:, :-incoming_tokens] = tensor[:, incoming_tokens:].clone()
...
def Recomp_Spec(tokenizer, target, target_cache, draft, input_ids, gamma=4, max_len=256, top_k=-1, top_p=0.9, temperature=0.6, verbose=False, file_path=None, dataset=None):
    ...
    if rolling_window.shape[1] < 256:
        blank_tensor = torch.zeros(rolling_window.shape[0], 256 - rolling_window.shape[1], dtype=rolling_window.dtype, device=rolling_window.device)
        rolling_window = torch.cat([blank_tensor, rolling_window], dim=1)

    next_token = sample(norm_logits(outputs.logits[:,-1,:], temperature=temperature ,top_k=top_k, top_p=top_p))
    rolling_window = shift_tensor(rolling_window, next_token.unsqueeze(0))
    if verbose:
        spec_stream(next_token[0], tokenizer, 'cyan')
