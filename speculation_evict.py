...
if args.cache == 'h2o':
    ...
    raise NotImplementedError(f"Cache {args.cache} not implemented!")
...
for input_ids in tokenized_prompts:
    ...
    with torch.no_grad():
        ...
        for i in tqdm(range(iter_prefill)):
            ...
            if not isinstance(past_key_values_q, SimpleCache):
                past_key_values_q.evict(100)
            outputs_q = model_q(
        ...
        for i in range(1):
            ...
            while n < max_len:
                ...
                if not isinstance(past_key_values_q, SimpleCache):
                    past_key_values_q.evict(gamma + 1)
                    # past_key_values_q.print_status()
                ...
                # print(past_key_values.seq_len, past_key_values_q.seq_len)
                # print(past_key_values.value_cache[-1][0, -1, past_key_values.seq_len-2])
                # print(past_key_values_q.value_cache[-1][0, -1, past_key_values_q.seq_len-1])
                # exit()
                ...
                for i, speculation_prob, verify_prob in zip(generated_ids, speculation_probs, verify_probs):
                    ...
                    # print(verify_prob[i], speculation_prob[i])
                ...
                if gamma == count:
                    outputs = model_q(
                        input_ids=verify_tokens[:,-1:].to(model_q.device),
                        past_key_values=past_key_values_q,
                        use_cache=True,
                    )
                else:
                ...
                # if past_key_values.seq_len != past_key_values_q.seq_len:
                #     print(past_key_values.seq_len, past_key_values_q.seq_len)
                # print(past_key_values.seq_len, past_key_values_q.seq_len)
                # exit()
