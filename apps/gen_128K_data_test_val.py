...
import json
json_file = open('/home/hanshis/workspace/LongContextInfer/data/pg19/pg19-test.json', "a") 

for i in range(50):
    prompt = dataset['validation'][i]['text'][:1024*128*5]
...
for i in range(100):
    prompt = dataset['test'][i]['text'][:1024*128*5]
    inpu_ids = tokenizer.encode(prompt, return_tensors='pt')
    if inpu_ids.shape[-1] > 128*1024:
        inpu_ids = inpu_ids[:, :129*1024]
        new_output = tokenizer.decode(inpu_ids[0], skip_special_tokens=True)
